...
pub struct ServerConfig {
    ...
    /// ECH key material for decrypting `encrypted_client_hello` offers.
    ///
    /// When set, clients that offer ECH with a matching `config_id` have
    /// their inner client hello decrypted and used for the handshake.
    /// Multiple keys support rotation during DNS TTL transitions.
    pub(crate) ech_keys: Option<Arc<Vec<super::ech::EchKeys>>>,
}

impl ServerConfig {
    /// Enable server-side Encrypted Client Hello (ECH) for this configuration.
    ///
    /// Each [`ech::EchKeys`] entry pairs an `ECHConfig` with the private key
    /// that matches its advertised public key.  When a client offers an
    /// `encrypted_client_hello` extension whose `config_id` matches one of
    /// the entries, the inner client hello is decrypted and used for the
    /// handshake; the client-facing certificate is then selected for the
    /// inner (real) server name.
    ///
    /// # Errors
    ///
    /// Returns an error when `keys` is empty.
    pub fn with_ech_keys(
        mut self,
        keys: Vec<super::ech::EchKeys>,
    ) -> Result<Self, Error> {
        if keys.is_empty() {
            return Err(Error::InvalidEncryptedClientHello(
                crate::error::EncryptedClientHelloError::InvalidConfigList,
            ));
        }
        self.ech_keys = Some(Arc::new(keys));
        Ok(self)
    }
...
pub struct ServerConnectionData {
    ...
    /// Whether the current handshake accepted an ECH offer.
    pub(super) ech_accepted: bool,
    /// Whether the client offered ECH (regardless of acceptance).
