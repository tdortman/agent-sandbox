...
pub(super) struct ExpectClientHello {
    ...
    /// ECH handshake state, when the first client hello was accepted.
    pub(super) ech: Option<super::ech::EchHandshakeState>,
}
...
impl State<ServerConnectionData> for ExpectClientHello {
    ...
    where
        Self: 'm,
    {
        // Attempt to accept an Encrypted Client Hello offer before any other
        // processing: an accepted offer replaces the client hello entirely.
        if self.config.ech_keys.is_some() {
            let mut this = self;
            let ech = if this.done_retry {
                this.ech.take()
            } else {
                None
            };
            let keys: &[super::ech::EchKeys] = this
                .config
                .ech_keys
                .as_deref()
                .map(Vec::as_slice)
                .unwrap_or(&[]);
            let outcome = super::ech::process_client_hello_ech(
                keys,
                &m,
                this.done_retry,
                ech,
            ...
            if outcome.offered && outcome.accepted.is_none() {
                // The client offered ECH but no key could decrypt it; the
                // outer hello is used, and the EncryptedExtensions
                // `retry_configs` tells the client about current keys.
                cx.data.ech_offered = true;
            }
            if let Some(accepted) = outcome.accepted {
                // The inner hello becomes the client hello for the rest of
                // the handshake.  The transcript buffer is fresh: the TLS 1.3
                // handler adds the (inner) hello message itself.
                let mut transcript_buffer = HandshakeHashBuffer::new();
                if this.config.verifier.offer_client_auth() {
                    transcript_buffer.set_client_auth_enabled();
                }
                this.transcript = HandshakeHashOrBuffer::Buffer(transcript_buffer);
                this.ech = Some(accepted.state);
                cx.data.ech_accepted = true;

                let (client_hello, sig_schemes) =
                    process_client_hello(&accepted.message, this.done_retry, cx)?;
                return this.with_certified_key(sig_schemes, client_hello, &accepted.message, cx);
            }
            // ECH was offered but not accepted: continue with the outer hello.
            let (client_hello, sig_schemes) = process_client_hello(&m, this.done_retry, cx)?;
            return this.with_certified_key(sig_schemes, client_hello, &m, cx);
        }
