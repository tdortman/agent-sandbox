...
use crate::msgs::handshake::{
    ...
    HandshakePayload, NewSessionTicketPayloadTls13, ServerEncryptedClientHello,
};
...
mod client_hello {
    ...
    pub(in crate::server) struct CompleteClientHelloHandling {
        ...
        /// ECH handshake state, when the client hello was accepted.
        pub(in crate::server) ech: Option<crate::server::ech::EchHandshakeState>,
    }
    ...
    fn emit_server_hello(
        ...
        config: &ServerConfig,
        ech: Option<&crate::server::ech::EchHandshakeState>,
        ...
        let server_random = if let Some(ech) = ech {
            // RFC 9849 section 7.2: the last 8 bytes of ServerHello.random
            // confirm ECH acceptance.  The confirmation is derived from a
            // transcript of the inner hello and a ServerHello whose random
            // ends in zeros; the real ServerHello carries the confirmation.
            let mut zeroed = randoms.server;
            zeroed[24..].fill(0);
            let sh_zeroed = Message {
                version: ProtocolVersion::TLSv1_2,
                payload: MessagePayload::handshake(HandshakeMessagePayload(
                    HandshakePayload::ServerHello(ServerHelloPayload {
                        legacy_version: ProtocolVersion::TLSv1_2,
                        random: Random::from(zeroed),
                        session_id: *session_id,
                        cipher_suite: suite.common.suite,
                        compression_method: Compression::Null,
                        extensions: Box::new(ServerExtensions {
                            key_share: Some(KeyShareEntry::new(ckx.group, ckx.pub_key.clone())),
            ...
            let mut confirmation = transcript.clone();
            confirmation.add_message(&sh_zeroed);
            let confirmation = crate::tls13::key_schedule::server_ech_confirmation_secret(
                suite.hkdf_provider,
                &ech.inner_random,
                confirmation.current_hash(),
            );
            let mut patched = zeroed;
            patched[24..].copy_from_slice(&confirmation);
            patched
        } else {
            randoms.server
        };
    ...
    fn emit_hello_retry_request(
        ...
        let make_req = |ech_conf: Option<Vec<u8>>| HelloRetryRequest {
            legacy_version: ProtocolVersion::TLSv1_2,
            session_id,
            cipher_suite: suite.common.suite,
            extensions: HelloRetryRequestExtensions {
                ...
                encrypted_client_hello: ech_conf.map(Payload::new),
                ..Default::default()
            },
        };

        // RFC 9849 section 7.2.1: the confirmation is computed over the
        // transcript of the first inner hello and an HRR whose ECH extension
        // payload is eight zero bytes; the real HRR carries the confirmation.
        let req = if let Some(ech) = ech {
            let zeroed = make_req(Some(vec![0; 8]));
            let zeroed_msg = Message {
                version: ProtocolVersion::TLSv1_2,
                payload: MessagePayload::handshake(HandshakeMessagePayload(
                    HandshakePayload::HelloRetryRequest(zeroed),
                )),
            };
            let mut confirmation = transcript.clone();
            confirmation.rollup_for_hrr();
            confirmation.add_message(&zeroed_msg);
            let confirmation = crate::tls13::key_schedule::server_ech_hrr_confirmation_secret(
                suite.hkdf_provider,
                &ech.inner_random,
                confirmation.current_hash(),
            );
            make_req(Some(confirmation.to_vec()))
        } else {
            make_req(None)
        };
    ...
    fn emit_encrypted_extensions(
        ...
        // RFC 9849 section 7.1: a server with ECH keys that rejected the
        // client's offer advertises its current configurations so the client
        // can retry with up-to-date keys.  Clients that never offered ECH
        // must not receive the extension.
        if cx.data.ech_offered && !cx.data.ech_accepted {
            if let Some(keys) = &config.ech_keys {
                ep.extensions.encrypted_client_hello_ack = Some(ServerEncryptedClientHello {
                    retry_configs: keys
                        .iter()
                        .map(crate::server::ech::EchKeys::config_payload)
                        .collect(),
                });
            }
        }
