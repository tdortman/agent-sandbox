...
/// A config builder state where the caller must supply how to provide a server
/// certificate to the connecting peer.
///
...
impl ConfigBuilder<ServerConfig, WantsServerCert> {
    ...
    /// extension to describe, e.g., the valid DNS name. The `commonName` field
    /// is disregarded.
    ///
    /// `cert_chain` is a vector of DER-encoded certificates.
    /// `key_der` is a DER-encoded private key as PKCS#1, PKCS#8, or SEC1. The
    /// `aws-lc-rs` and `ring` [`CryptoProvider`][crate::CryptoProvider]s
    /// support all three encodings, but other `CryptoProviders` may not.
    ///
