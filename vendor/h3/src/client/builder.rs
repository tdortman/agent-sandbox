...
impl Builder {
    ...
    /// Enables the `SETTINGS_ENABLE_WEBTRANSPORT` setting.
    pub fn enable_webtransport(&mut self, enabled: bool) -> &mut Self {
        self.config.settings.enable_webtransport = enabled;
        self
    }

    /// Enables the extended CONNECT protocol required for various HTTP/3
    /// extensions.
        ...
        B: Buf,
    {
        ...
        Ok((
            Connection {
                ...
                sent_closing: None,
                recv_closing: None,
                accept_incoming_bi: self.config.settings.enable_webtransport(),
            },
            send_request,
        ))
    }
