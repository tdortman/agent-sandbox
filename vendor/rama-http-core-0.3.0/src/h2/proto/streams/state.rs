...
enum Cause {
    ...
    /// - Header block size was too large, so we want to REFUSE, possibly after
    ///   sending a 431 response frame.
...
impl State {
    ...
    pub(super) fn ensure_reason(
        ...
        match self.inner {
            Inner::Closed(
                Cause::Error(Error::Reset(_, reason, _) | Error::GoAway(_, reason, ..))
