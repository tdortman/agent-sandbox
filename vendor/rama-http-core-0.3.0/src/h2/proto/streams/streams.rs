#![expect(
    ...
    reason = "vendored from upstream `h2`: many `&mut self` methods preserve API \
              symmetry/locking-position semantics that the `&self` form would lose"
)]
...
use rama_http::proto::{RequestHeaders, h2::frame::EarlyFrameStreamContext};
use rama_http_types::{
    HeaderMap, Request, Response,
    proto::h2::{
        PseudoHeaderOrder,
        ext::Protocol,
        frame::{self, Frame, Reason, Settings},
    },
};
...
// Like `Streams` but with a `peer::Dyn` field instead of a static `P: Peer`
// type parameter. Ensures that the methods only get one instantiation, instead
// of two (client and server)
#[derive(Debug)]
    ...
    P: Peer,
{
            ...
            Some(early_frame) => {
                let frame: Frame<Prioritized<B>> = match early_frame {
                    ...
                    frame::EarlyFrame::Settings(settings) => {
                        // NOTE: if ever issues, we might need to do something locally with settings
                        // as well
                        return Poll::Ready(Ok((!settings.flags.is_ack()).then_some(settings)));
                    }
                    // NOTE: this can cause issues in case regular logic would also send a windows
                    // update soon afterwards
                    frame::EarlyFrame::WindowUpdate(window_update) => {
                        ...
                        if let Err(reason) = me.actions.recv.replay_connection_window_update(
                            ...
                            tracing::debug!(
                                "h2 streams: failed to replay window update \
                                 (replay_connection_window_update): reason = {reason}; go away"
                            );
    ...
    pub(crate) fn send_request(
        ...
        .map_err(|err| {
            tracing::warn!(
                "failed to create stream for id {stream_id:?} in send_request: {err}; library GO \
                 AWAY"
            );
            ...
            Entry::Vacant(e) => {
                ...
                match self.actions.recv.open(id, Open::Headers, &self.counts)? {
                    Some(stream_id) => {
                        ...
                        .map_err(|err| {
                            tracing::warn!(
                                "failed to create recv stream for id {stream_id:?}: {err}; \
                                 library GO AWAY"
                            );
                            Error::library_go_away(err)
                        })?;
    ...
    fn recv_push_promise<B>(
        ...
        let child_key: Option<store::Key> = {
            ...
            let stream = self.store.insert(promised_id, {
                ...
                .map_err(|err| {
                    tracing::warn!(
                        "failed to create pushed stream for promised id {promised_id:?}: {err}; \
                         library GO AWAY"
                    );
                    Error::library_go_away(err)
                })?
    ...
    fn send_reset<B>(
        ...
        let key = match self.store.find_entry(id) {
            ...
            Entry::Vacant(e) => {
                ...
                //
                // 1. As a server, we just received a request, but that request was bad, so
                //    we're resetting before even accepting it. This is totally fine.
                //
                // 2. The remote may have sent us a frame on new stream that it's *not* supposed
                //    to have done, and thus, we don't know the stream. In that case, sending a
                //    reset will "open" the stream in our store. Maybe that should be a
                //    connection error instead? At least for now, we need to update what our
                //    vision of the next stream is.
                ...
                match Stream::try_new(id, 0, 0, &self.extensions) {
                    ...
                    Err(reason) => {
                        tracing::debug!(
                            "h2 streams: failed to create new stream in send_reset: reason = \
                             {reason}; go away"
                        );
...
impl<B> StreamRef<B> {
    ...
    pub fn send_informational_headers(&mut self, frame: frame::Headers) -> Result<(), UserError> {
        ...
        me.counts.transition(stream, |counts, stream| {
            ...
            // Send the interim informational headers directly to the buffer without state
            // changes This bypasses the normal send_headers flow that would
    ...
    pub(crate) fn send_response(
        ...
        // We need to only drop extensions after we release our locks or there is risk
        // for deadlocking
    ...
    pub(crate) fn send_push_promise(&mut self, request: Request<()>) -> Result<Self, Error> {
        // We need to keep extensions around so they are dropped after our locks or we
        // risk deadlocking
        ...
        let child_key = {
            let mut child_stream = me.store.insert(
                ...
                .map_err(|err| {
                    tracing::warn!(
                        "failed to create stream for promised id {promised_id:?} in \
                         send_push_promise: {err}; library GO AWAY"
                    );
                    Error::library_go_away(err)
                })?,
            );
    ...
    /// Request to be notified for if a `RST_STREAM` is received for this
    /// stream.
...
impl OpaqueStreamRef {
    ...
    /// Called by a client to check for informational responses (1xx status
    /// codes)
    ...
    /// Clear the receive queue and set the status to no longer receive data
    /// frames.
...
fn maybe_cancel(stream: &mut store::Ptr, actions: &mut Actions, counts: &mut Counts) {
    if stream.is_canceled_interest() {
        // Server is allowed to early respond without fully consuming the client input
        // stream But per the RFC, must send a RST_STREAM(NO_ERROR) in such cases. https://www.rfc-editor.org/rfc/rfc7540#section-8.1
...
impl Actions {
    ...
    /// Check if we possibly could have processed and since forgotten this
    /// stream.
    ///
