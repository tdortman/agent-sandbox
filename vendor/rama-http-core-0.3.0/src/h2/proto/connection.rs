...
use rama_core::{bytes::Bytes, extensions::ExtensionsRef, futures::Stream, telemetry::tracing};
use rama_http::proto::h2::frame::EarlyFrameStreamContext;
use rama_http_types::proto::h2::{
    PseudoHeaderOrder,
    frame::{DEFAULT_INITIAL_WINDOW_SIZE, Reason, StreamId},
};
use tokio::io::AsyncRead;

use crate::h2::{client, codec::UserError, proto::*, server};
...
// Extracted part of `Connection` which does not depend on `T`. Reduces the
// amount of duplicated method instantiations.
#[derive(Debug)]
    ...
    B: Buf,
{
    ...
    pub(crate) fn try_new(
        ...
        // Transfer ownership of extensions to Streams as at this point our connection
        // is esthablished and we only need these extensions as parents for our
        // inner Stream's
    ...
    fn take_error(&mut self, ours: Reason, initiator: Initiator) -> Result<(), Error> {
        ...
        match (ours, theirs) {
            ...
            (ours, Reason::NO_ERROR) => Err(Error::GoAway(Bytes::new(), ours, initiator, false)),
    ...
    B: Buf,
{
    ...
    fn handle_poll2_result(&mut self, result: Result<(), Error>) -> Result<(), Error> {
        match result {
            ...
            Err(Error::GoAway(debug_data, reason, initiator, _)) => {
            ...
            Err(Error::Io(kind, inner)) => {
                ...
                // Attempting to read after the client dropped the connection results in
                // UnexpectedEof If as a server, we don't have anything more to
                // send, just close the connection without error
                //
    ...
    fn handle_go_away(&mut self, reason: Reason, debug_data: Bytes, initiator: Initiator) {
        let e = Error::GoAway(debug_data.clone(), reason, initiator, false);
