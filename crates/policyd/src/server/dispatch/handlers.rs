...
use agent_sandbox_core::{
    RegisterUiReply, RequestContext, ResolvedRequestContext, RpcReply, RpcRequest, SimpleOkReply,
    split_check_aliases,
};
    ...
    match req {
        ...
        RpcRequest::CheckDbus { target, ctx } => {
            let ctx = if role == SocketRole::Host {
                store.resolve_dbus_proxy_context(
        ...
        RpcRequest::StartFilesystemMonitor { ctx, static_allow } => {
            ...
            Ok(RpcReply::FilesystemMonitor(
                store
                    .start_filesystem_monitor(peer_pid, ctx, static_allow)
                    .await,
            ))
        }
