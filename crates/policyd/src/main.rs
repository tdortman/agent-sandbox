...
struct Cli {
    ...
    /// agent-sandbox-ui) connect here to register and receive prompts.
    #[arg(
        long,
        value_name = "SOCKET",
    ...
    /// Sandbox-side Unix socket path. Components running inside the bwrap mount
    /// namespace (agent-sandbox-nfq, agent-sandbox-syscall-arm,
    /// agent-sandbox-fs-arm) connect here. Must differ from the host socket.
    #[arg(
        long,
        value_name = "SOCKET",
    ...
    /// Trusted publisher's pinned map for experimental network-grant
    /// revocation.
    #[arg(long, value_name = "PIN")]
    network_revocation_map: Option<PathBuf>,

    /// Declarative policy JSON file. The lowest-priority policy layer,
    /// typically generated by the NixOS module from "agent-sandbox.declarative"
    /// settings.
    #[arg(
        long,
        value_name = "PATH",
    ...
    /// Optional path to also export the merged policy as a Nix attribute set
    /// expression. Empty (the default) disables the Nix export. Used by NixOS
    /// rebuilds to regenerate "environment.etc.\"agent-sandbox/policy.nix\"".
    #[arg(long, value_name = "PATH", default_value = "")]
    export_nix: String,

    /// How long (in seconds) to wait for a UI client to answer an approval
    /// prompt before denying the request. Fractional values are accepted. The
    /// effective wait is clamped to at least 1 second.
    #[arg(long, value_name = "SECONDS", default_value = "300")]
    approval_timeout: f64,

    /// Allow UI clients to receive interactive approval prompts. When disabled,
    /// all non-declarative requests are denied without prompting. Useful for
    /// headless or CI sandboxes.
    #[arg(long = "no-interactive-approval", action = clap::ArgAction::SetFalse, default_value_t = true)]
    interactive_approval: bool,

    /// Path to the "agent-sandbox-ui" binary. If unset, the daemon falls back
    /// to the env var `AGENT_SANDBOX_UI_SPAWN_CMD`, then to a built-in default.
    /// Used when policyd needs to spawn a transient UI client for a request
    /// that arrived with no registered UI.
    #[arg(long, value_name = "PATH", env = "AGENT_SANDBOX_UI_SPAWN_CMD")]
    ui_spawn_cmd: Option<PathBuf>,

    /// Path to the "agent-sandbox-fsmon" binary. If unset, the daemon falls
    /// back to the env var `AGENT_SANDBOX_FS_MONITOR_CMD`. Used when an
    /// "agent-sandbox-fs-arm" request asks for a fanotify monitor to be started
    ...
    /// user-notification handler to be spawned.
