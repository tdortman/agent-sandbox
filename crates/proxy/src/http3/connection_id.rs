...
use crate::{
    http3::{BoxError, boxed_owned, varint, ConnectionIdOwner},
    policy::FlowClaim,
};
use h3::error::Code;
...
impl ConnectionIdBindings {
    ...
    pub(super) fn drain_or_close(&mut self, connection: &quinn::Connection) -> Result<(), BoxError> {
    ...
    pub(super) fn retire(&mut self, sequence: u64, id: quinn::ConnectionId) -> Result<(), BoxError> {
...
mod tests {
    ...
    fn drop_releases_every_bound_id() {
        ...
        let second = owner();
        registry.bind(cid, second).expect("drop released the bound id");
    }
}
